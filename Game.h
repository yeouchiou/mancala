...
public:
	...
	// show the names of the players and a reasonable representation of the state of the board. 
	void status(bool& over, bool& hasWinner, Side& winner) const;
	// If the game isn't over (i.e., more moves are possible), set over to false 
	// and do not change anything else. Otherwise, set over to true 
	// and hasWinner to true if the game has a winner, or false if it resulted in a tie. 
	// If hasWinner is set to false, leave winner unchanged; otherwise, set it to the winning side. 
	bool move();
	// If the game is over, return false.
	// Otherwise, make a complete move for the player whose turn it is
	...
	// If the player gets an additional turn, you should display the board so someone 
	// looking at the screen can follow what's happening.
	void play();
	...
	// scrolling through the whole game, it would be reasonable periodically to prompt
	// the viewer to press ENTER to continue and not proceed until ENTER is pressed. 
